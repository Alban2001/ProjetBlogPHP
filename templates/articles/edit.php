        ...
        <div class="row">
            <div class="col">
                ...
                <p><strong>Date de création</strong> :
                    <?php echo htmlspecialchars($article->getDateCreation()->format("d/m/Y"), ENT_COMPAT, 'utf-8'); ?>
                </p>
                ...
                <form action="index.php?action=retourEditArticle" method="POST" enctype='multipart/form-data'>
                    ...
                    <input class="inputAjoutArticle form-control border border-3" type="text" name="title"
                        placeholder="Titre" maxlength="255" value="<?php if (isset($inputs["title"]) && !empty($inputs["title"])) {
                            ...
                            echo htmlspecialchars($article->getTitre(), ENT_COMPAT, 'utf-8');
                        } ?>">
                    ...
                    <input id="inputFileImage" class="form-control border border-3" type="file" name="image"
                        ...
                        value="<?php echo 'images/upload/' . htmlspecialchars($article->getImage(), ENT_COMPAT, 'utf-8'); ?>">
                    ...
                    if (isset($inputs["chapo"]) && !empty($inputs["chapo"])) {
                        $chapo = htmlspecialchars($inputs["chapo"], ENT_COMPAT, 'utf-8');
                    } else {
                        $chapo = htmlspecialchars($article->getChapo(), ENT_COMPAT, 'utf-8');
                    } ?>
                    ...
                    if (isset($inputs["content"]) && !empty($inputs["content"])) {
                        $content = htmlspecialchars($inputs["content"], ENT_COMPAT, 'utf-8');
                    } else {
                        $content = htmlspecialchars($article->getContenu(), ENT_COMPAT, 'utf-8');
                    } ?>
